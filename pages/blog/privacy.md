...
<ol>
	<li>
		...
		<p>Installed applications may be randomly accessing certain areas which are known to expose personal identity, such as cameras and microphone. Thankfully, you can make it so that colored dots may appear on the top right of the screen when they're being accessed. iOS 14+ has a bundled solution but it is locked to green for camera and orange for microphone. On Android, you could download <a href="https://play.google.com/store/apps/details?id=you.in.spark.access.dots">Access Dots</a> which can customize the colors and position of the dots.</p>
		<p>What better way to download apps from the Google Play Store on an Andoird phone than by using <a href="https://f-droid.org/en/packages/com.aurora.store/">Aurora Store</a>? There is no need to link your Google account (and it's encouraged of you not to do so) and you can check trackers and permissions the app would need <b>before</b> the app get's installed. No app is safe from the store's exposure, regardless of which country it originated from.</p>
